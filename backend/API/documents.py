...
@router.put("/{doc_id}",
            ...
            status_code=status.HTTP_200_OK)
def update_document(*,session:SessionDep,document:AvailableDoc,file:Annotated[UploadFile, File(...)])->DocumentPublic:
    update_data = {
        "filename": file.filename,
        "uploaded_at": datetime.now(timezone.utc)
    }
    s3_utils.update_s3_file_object(settings.S3_BUCKET_NAME, file, document.s3_key)
    doc_updated=crud_db.update_document(db_session=session,document_in=document,update_info=update_data)
    return doc_updated

@router.delete("/{doc_id}",
               status_code=status.HTTP_204_NO_CONTENT)
def delete_document(*,session:SessionDep,document:AvailableDoc)->None:
    s3_utils.delete_s3_file_object(settings.S3_BUCKET_NAME,document.s3_key)
