...
def is_owner(member:ActiveMember)->db_model.ProjectMember:
    if member.role!=ProjectAccess.owner:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not enough permissions, you are not the project's owner.")
    return member

ProjectOwner=Annotated[db_model.ProjectMember,Depends(is_owner)]

def get_actual_document(
        session:SessionDep,
        doc_id:uuid.UUID,
        current_user:CurrentUser)->db_model.Document:
    document = crud_db.get_document_by_id(
        db_session=session,
        document_id=doc_id)
    if not document:
        raise HTTPException(
    ...
    if not crud_db.authenticate_project_member(
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You are not a member of the project this document belongs to.")
    return document
